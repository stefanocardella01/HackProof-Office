...
public class PCCameraControllerM4 : MonoBehaviour
{
    ...
    public void TransitionToScreen()
    {
        ...
        if (isTransitioning)
        {
            Debug.LogWarning("[PCCameraController] Gi� in transizione, ignoro");
            return;
        }

        if (isAtScreen)
        {
            Debug.LogWarning("[PCCameraController] Gi� allo schermo, ignoro");
            return;
        }
    ...
    private IEnumerator TransitionBackCoroutine()
    {
        ...
        // Ripristina la Event Camera (opzionale, ma pi� pulito)
